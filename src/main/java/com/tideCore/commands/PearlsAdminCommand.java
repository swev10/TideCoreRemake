    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        switch (action) {
            case "give" -> {
                ...
                sender.sendMessage(MessageUtils.prefix() + "§aGave §e" + amount + " pearls §ato §b" + target.getName());
            }
            case "set" -> {
                PlayerDataManager.setPearls(target, amount);
                sender.sendMessage(MessageUtils.prefix() + "§aSet §b" + target.getName() + "§a's pearls to §e" + amount);
            }
            case "take" -> {
                PlayerDataManager.takePearls(target, amount);
                sender.sendMessage(MessageUtils.prefix() + "§aTook §e" + amount + " pearls §afrom §b" + target.getName());
            }
            default -> {
                sender.sendMessage(MessageUtils.prefix() + "§cUsage: /tidecore pearls <give|set|take> <player> <amount>");
            }
